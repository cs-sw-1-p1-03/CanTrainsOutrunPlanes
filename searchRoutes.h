...
void searchRoutes(char arrivalCity[], char departureCity[], route_t arrayOfRoutes[], routeFile_t routeFileArray[])
{
    ...
    for (int i = 0; i < 1; i++) { // only for flight
        ...
        departureIndex = -1;
        arrivalIndex = -1; // flight: the trip is complete and variables reset to -1.
    }

    for (int i = 1; i < numberOfRoutes; i++) {           //train: for every list.  (reading down the list is ++ )
        for (int j = 0; j < routeFileArray[i].length; j++) { //for every line
            if (strcmp(arrayOfRoutes[i].list[j].departureCity, departureCity) == 0) {
                departureIndex = j;
            }
        ...
        if (departureIndex >= 0 && arrivalIndex >= 0) {
            ...
            if (departureIndex > arrivalIndex) {
                ...
                for (int k = departureIndex - 1; k > arrivalIndex + 0; k--) {  // (reading up the list is --)
                ...
                // list is read downwards
                    for (int p = departureIndex; p < arrivalIndex + 1; p++) { // p++: otherwise it reads down the list. Arrival needs to be smaller than departure in the list to choose the correct line.
                      //Adding all the time, saved from the list
