...
void scanChar(char input[],arrayOfStrings_t match[],int arrayLength,char text[]){
    ...
    }while(strcmp(input,"exit")!=0 && found == 0); // "exit" it skips the step and NOT the program.
 ...
 */
bool digitCheck(char key[]) // key is the input from scanNNumber
{
...
int scanNumber(char text[]){
    char input[5]; // input is char which is converted to an integer using atoi. If typing letters it wont work, and asks again.
    do {
        printf("%s",text);
        scanf("%5s", input);
